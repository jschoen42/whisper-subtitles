...
def check_zip(myzip, path: str, files: list) -> dict[str]:
    errors = {}
    for file in files:
        try:
...
def expand_zip(source_path: str, dest_path: str) -> bool:
...
def create_zip(source_path: str, dest_path: str, filename: str, compression = 6) -> bool:
    ...
    src_path = Path(source_path).expanduser().resolve(strict=True)
    try:
