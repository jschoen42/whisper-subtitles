...
class WhisperModel:
    def __init__(
        ...
        **model_kwargs,
        ...
        self.max_length = 448 ### 448 prompt.extend(previous_tokens[-(self.max_length // 2 - 1) :]) -> max prompt length = 224 + 3 = 227 # JS
                              ### https://github.com/openai/whisper/discussions/1824 ->  the prompt is limited to only 224 tokens        # JS
                              ### https://platform.openai.com/docs/guides/speech-to-text/prompting -> 224 token of the prompt            # JS
                              ### https://cookbook.openai.com/examples/whisper_prompting_guide -> 224                                    # JS
    ...
    def transcribe(
        ...
        self.logger.info(
            "[transcribe] Processing audio with duration %s", format_timestamp(duration) # JS
        )
        ...
        if vad_filter and clip_timestamps == "0":
            ...
            self.logger.info(
                "[transcribe] VAD filter removed %s of audio",  # JS
                format_timestamp(duration - duration_after_vad),
            )

            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    "[transcribe] VAD filter kept the following audio segments: %s", # JS
                    ", ".join(
                        "[%s -> %s]"
                        % (
        ...
        if language is None:
            if not self.model.is_multilingual:
                ...
                self.logger.info(
                    "[transcribe] Detected language '%s' with probability %.2f", # JS
                    language,
                    language_probability,
                )
        else:
            if not self.model.is_multilingual and language != "en":
                self.logger.warning(
                    "[transcribe] The current model is English-only but the language parameter is set to '%s'; " # JS
        ...
        return segments, info, sampling_rate, speech_chunks # JS added: sampling_rate, speech_chunks
    ...
    def generate_segments(
        ...
        while clip_idx < len(seek_clips):
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    "[generate_segments] Processing segment at %s", format_timestamp(time_offset) # JS
                )
            ...
            if seek > 0 and segment_size < 3000:                                                        # JS
                special_lastsegment = True                                                              # JS
            ...
            (
                ...
                compression_ratio,
                result_log, # JS
            ) = self.generate_with_fallback(encoder_output, prompt, tokenizer, options, seek, special_lastsegment) # JS
            ...
            my_prompt = {}                                  # JS
            my_prompt["text"]   = tokenizer.decode(prompt)  # JS
            my_prompt["length"] = len(prompt)               # JS
            my_prompt["token"]  = prompt                    # JS
            ...
            if (
                ...
                if options.condition_on_previous_text:
                    self.logger.debug(
                        "[generate_segments] Reset prompt. prompt_reset_on_temperature threshold is met %f > %f", # JS
                        temperature,
    ...
    def generate_with_fallback(
        ...
        options: TranscriptionOptions,
        seek: int,
        special_lastsegment: bool, # JS
    ) -> Tuple[ctranslate2.models.WhisperGenerationResult, float, float, float, dict]: # JS add , dict
        decode_result = None
        all_results = []
        ...
        if self.logger.isEnabledFor(logging.DEBUG):                                # JS
            self.logger.debug(                                                     # JS
                f"[generate_with_fallback] prompt: '{tokenizer.decode(prompt)}'"   # JS
            )                                                                      # JS

        for temperature in options.temperatures:
            if self.logger.isEnabledFor(logging.DEBUG):                             # JS
                self.logger.debug(                                                  # JS
                    f"[generate_with_fallback] current temperature: {temperature}"  # JS
                )                                                                   # JS
            ...
            if (
                ...
                decode_result = (         # JS
                    decode_result[0],     # JS
                    -99,                  # JS was: avg_logprob, (JS Patch, otherwise halucinated result reappears)
                    temperature,          # JS
                    decode_result[3],     # JS
                    result_log,           # JS
                )                         # JS
    ...
    def get_prompt(
        ...
        if previous_tokens or (hotwords and not prefix):
            ...
            if hotwords and not prefix:
                ...
                tmp = hotwords_tokens[: self.max_length // 2 - 4] # JS: 4
                                                                  # Hotwords nicht aktiv
            if previous_tokens:
                tmp = previous_tokens[-(self.max_length // 2 - 4) :] # JS: 4
    ...
    def add_word_timestamps(
        ...
        for segment, text_tokens in zip(segments, text_tokens_per_segment):
            ...
            while word_index < len(alignment) and saved_tokens < len(text_tokens):
                ...
                word = timing["word"] # JS
                if word:  # JS
                    if word_index == 0 and word[0] != " ": # JS patch 16.09.2024
                        word = " " + word                  # JS
